...
describe('function', function() {
  /*TEST ONE: it should return the player bet if he has a pair comb, given the cards on the table
    it('should return 40 as bet answer', function(){
      // firstBet(total,ingame,ontablefish, result, random,i)
...
/*TEST ONE: it should return bet for the second round based on the fish on table and a pair in hand*/
      it('should return 40 as bet answer', function(){
      // cpuSecondRound(difference,previous, ingame[i].textContent, total[i].textContent,ontablefish,result[i])
      assert.equal(cpuSecondRound(0, "30", "30", "110", ["40", "30"],1), 40);
      });
/*TEST TWO: it should return bet for the second round based on the fish on table and a straigth in hand*/
      it('should return 60 as bet answer', function(){
        // cpuSecondRound(difference,previous, ingame[i].textContent, total[i].textContent,ontablefish,result[i])
        assert.equal(cpuSecondRound(0, "30", "30", "110", ["60", "30"],4), 60);
        });
  /*TEST THREE: it should return bet for the second round based on the fish on table and a fourofaKind in hand*/
      it('should return 60 as bet answer', function(){
        // cpuSecondRound(difference,previous, ingame[i].textContent, total[i].textContent,ontablefish,result[i])
        assert.equal(cpuSecondRound(0, "30", "30", "60", ["60", "30"],7), 60);
        });
  /*TEST FOUR: it should return a no-bet decision for the second round based on the fish on table and a pair in hand*/
      it('should return 40 as bet answer', function(){
        // cpuSecondRound(difference,previous, ingame[i].textContent, total[i].textContent,ontablefish,result[i])
