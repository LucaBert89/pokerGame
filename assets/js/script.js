...
random = randomCard (the array that contains all the cards of the game*/
    let totalRanks;
function players(random, rank, suit, result) {
    ...
    The goal is to divide the cards among the players, 5 per player with their rank and suit*/
        ...
        handCombination(totalRanks,totalSuits, result); 
}
...
let cardNumber;
let key;
function handCombination(totalR, totalS, result) {
    ...
    for(let i=0; i<playerNumbers; i++) {
        ...
        countValues(totalR,count,i, totalCount);

        /*here the same I did for the rank but for the suit*/
        countValues(totalS,countSuit,i, totalSuit);
        ...
        straightComb = straigth(totalR[i], suitvalues, result);
